defmodule FusionFlowWeb.UserAuth do
  use FusionFlowWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  use Gettext, backend: FusionFlowWeb.Gettext
  ...
  @remember_me_cookie "_fusion_flow_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      FusionFlowWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      FusionFlowWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule FusionFlowWeb.PageLive do
        use FusionFlowWeb, :live_view

        on_mount {FusionFlowWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{FusionFlowWeb.UserAuth, :require_authenticated}] do
